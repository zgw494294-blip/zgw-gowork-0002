...
func TestRejectedEarlyReturnKeepsCopyReserved(t *testing.T) {
	store := storage.New()
	svc := New(store)
	...
	if err := svc.ReturnRequest("req1"); err == nil {
		t.Fatal("expected return before receipt to fail")
	}
	err := svc.CreateRequest(domain.BorrowRequest{ID: "req2", CopyID: "c1", ReaderID: "r2", Status: domain.RequestApplied, RequestedAt: time.Now()})
	if err == nil {
		t.Fatal("rejected return released a copy that is still in transit")
	}
}
